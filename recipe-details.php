...
// ============ FETCH RECIPE ============
try {
    $stmt = $pdo->prepare('SELECT r.*, u.name AS author_name FROM recipes r JOIN users u ON r.user_id = u.id WHERE r.id = ?');
    $stmt->execute([$id]);
    $recipe = $stmt->fetch();
...
<head>
    <title><?php echo $recipe ? htmlspecialchars($recipe['title']) : 'Recipe Not Found'; ?></title>
</head>
